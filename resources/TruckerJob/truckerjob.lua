...
function Chat(t)
	TriggerEvent("chatMessage", 'TRUCKER', { 0, 500, 500}, "" .. tostring(t))
end

--locations
--arrays
local TruckingCompany = {}
TruckingCompany[0] = {["x"] = -60.04, ["y"] = -2518.06, ["z"] = 7.4}
local TruckingTrailer = {}
TruckingTrailer = {
     [0] = {["x"] = -145.0, ["y"] = -2414.71, ["z"] = 6.0, ['h'] = 356.94},
     [1] = {["x"] = -51.97, ["y"] = -2415.67, ["z"] = 6.0, ['h'] = 262.83},
     [2] = {["x"] = -130.0, ["y"] = -2414.71, ["z"] = 6.0, ['h'] = 269.09}
...
function tick()
    ...
    if( GetDistanceBetweenCoords( playerCoords, TruckingCompany[0]["x"], TruckingCompany[0]["y"], TruckingCompany[0]["z"] ) < 01) then
...
function MISSION.spawnTruck()
    ...
    MISSION.truck = CreateVehicle(MISSION.hashTruck, -62.35, -2531.74, 6.25, 235.82, true, false)
    SetVehicleOnGroundProperly(MISSION.trailer)
    SetVehicleNumberPlateText(MISSION.truck, "TruckIt")
...
function MISSION.spawnTrailer()
    numTrailers = tablelength(TruckingTrailer)
    RandTrailer = TruckingTrailer[math.random(numTrailers) - 1]
    MISSION.trailer = CreateVehicle(MISSION.hashTrailer, RandTrailer["x"], RandTrailer["y"], RandTrailer["z"], RandTrailer["h"], true, false)
    SetVehicleOnGroundProperly(MISSION.trailer)
...
function MISSION.trailerMarker()
    ...
    SetBlipSprite(BLIP.trailer[BLIP.trailer.i], 466)
...
function MISSION.markerUpdate(trailerAttached)
    if( not BLIP.destination[BLIP.destination.i] and trailerAttached) then
       -- BLIP.destination.i = BLIP.destination.i + 1 this happens in GUI.mission()
        ...
        SetBlipSprite(BLIP.destination[BLIP.destination.i], 466)
    ...
    elseif ( not trailerAttached and BLIP.trailer[BLIP.trailer.i]) then
        SetBlipSprite(BLIP.trailer[BLIP.trailer.i], 466) --visible
        SetBlipColour(BLIP.trailer[BLIP.trailer.i], 17)
    end
    Wait(50)
end
